...
export default function AdminPanel() {
  ...
  const checkUserData = async () => {
    ...
    try {
      ...
      const dataInfo = {
        totalChickens: csvData.totalChickens || 0, // ✅ Inclure le total de poulets
        expenses: csvData.expenses.length,
        revenues: csvData.revenues.length,
        mortality: csvData.mortality.length,
        monthlyFinancials: csvData.monthlyFinancials.length,
        hasData: csvData.expenses.length > 0 || csvData.revenues.length > 0 || csvData.mortality.length > 0 || csvData.totalChickens > 0,
  ...
  const downloadUserCsv = async () => {
    ...
    try {
      ...
      csvContent += `# Total de poulets: ${csvData.totalChickens || 0}\n` // ✅ Ajout du total de poulets
      ...
      csvContent += `TOTAL_CHICKENS,${csvData.totalChickens || 0}\n`
      csvContent += `# \n`
