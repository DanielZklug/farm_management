...
export default function Dashboard() {
  ...
  const fetchUserDashboardData = async (userId) => {
    try {
      ...
      const totalRevenue = userData.farmData.revenue.reduce((sum, r) => sum + r.total_amount, 0)
      const totalExpenses = userData.farmData.expenses.reduce((sum, e) => sum + e.amount, 0)
      const totalMortalityLoss = userData.farmData.mortality.reduce((sum, m) => sum + m.estimated_loss, 0)
      const netProfit = totalRevenue - totalExpenses - totalMortalityLoss
      const reinvestmentAmount = netProfit > 0 ? netProfit * 0.7 : 0
      const totalMortality = userData.farmData.mortality.reduce((sum, m) => sum + m.count, 0)
      const mortalityRate = userData.farmData.totalChickens > 0 ? (totalMortality / userData.totalChickens) * 100 : 0
      ...
      setChickenCount(userData.farmData.totalChicken)
